...
json.category_title Category.find(restaurant.category_id).title
json.star (Review.where(Restaurant_id: restaurant.id).pluck(:star).map{|x| x.to_i}.inject(:+).to_f / Review.where(Restaurant_id: restaurant.id).count.to_f).round
